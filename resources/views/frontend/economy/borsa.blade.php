        ...
        <section style="display:grid;grid-template-columns:720px 320px;column-gap:20px;width:1060px;max-width:100%;">
            <div style="width:720px;">
                <section class="border bg-white rounded-lg overflow-hidden" style="width:100%;">
                    <div style="padding:14px 16px;border-bottom:1px solid #e5e7eb;">
                        ...
                        <h1 class="text-2xl font-semibold mt-1">Borsa</h1>
                    ...
                    <div style="max-height:560px;overflow:auto;padding:6px 16px 12px;">
                        @if(empty($economyAllMarkets))
                            <div class="text-sm text-gray-500 py-4">Veri yok.</div>
                        @else
                            <div style="display:grid;grid-template-columns:1fr 110px 80px 70px;gap:10px;align-items:center;font-size:11px;font-weight:700;color:#6b7280;padding:8px 0;border-bottom:1px solid #e5e7eb;text-transform:uppercase;">
                                <div>Hisse</div>
                                <div style="text-align:right;">Son</div>
                                <div style="text-align:right;">Deg%</div>
                                <div style="text-align:right;">Saat</div>
                            </div>
                            @foreach($economyAllMarkets as $item)
                                <div style="display:grid;grid-template-columns:1fr 110px 80px 70px;gap:10px;align-items:center;font-size:13px;padding:9px 0;border-bottom:1px solid #f3f4f6;">
                                    <div>
                                        <div style="font-weight:700;">{{ $item['symbol'] ?? '-' }}</div>
                                        <div style="color:#6b7280;">{{ \Illuminate\Support\Str::limit($item['name'] ?? '-', 42) }}</div>
                                    </div>
                                    <div style="text-align:right;">{{ $format($item['selling'] ?? $item['buying'] ?? null, 4) }}</div>
                                    <div style="text-align:right;color:{{ (($item['change'] ?? 0) < 0) ? '#b91c1c' : '#047857' }};">
                                        {{ $item['change'] !== null ? $format($item['change'], 2) . '%' : '-' }}
                                    </div>
                ...
                <section style="margin-top:20px;">
                    ...
                    <div class="text-center" id="economy-borsa-load-wrap">
                        @if($posts->hasMorePages())
                            <button id="economy-borsa-load-more" class="px-4 py-2 rounded border bg-white hover:bg-gray-50 text-sm">Daha Fazla Yukle</button>
                        @endif
                    </div>
                </section>
            </div>
            ...
            <aside style="width:320px;">
                ...
                <section class="space-y-5 sticky" style="top:84px;margin-top:16px;">
                    <div class="border bg-white rounded-lg flex items-center justify-center text-sm text-gray-400" style="height:100px;">320x100 Header Alti Reklam</div>
                    <div class="border bg-white rounded-lg flex items-center justify-center text-sm text-gray-400" style="height:250px;">320x250 Sidebar Reklam</div>
                    <div class="border bg-white rounded-lg flex items-center justify-center text-sm text-gray-400" style="height:600px;">300x600 Skyscraper Reklam</div>
                </section>
            </aside>
        </section>
    </div>
...
<script>
    (function () {
        const loadBtn = document.getElementById('economy-borsa-load-more');
        const feed = document.getElementById('economy-borsa-feed');
        const wrap = document.getElementById('economy-borsa-load-wrap');
        if (!loadBtn || !feed || !wrap) return;

        let nextPage = {{ $posts->hasMorePages() ? (int) ($posts->currentPage() + 1) : 'null' }};
        let pending = false;
        const baseUrl = @json(route('category.economy.borsa', $category->slug));
        ...
        loadBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                if (!nextPage) wrap.remove();
                else { loadBtn.disabled = false; loadBtn.textContent = 'Daha Fazla Yukle'; }
            } catch (e) {
