            ...
            <div style="width:720px;">
                <section class="border bg-white rounded-lg overflow-hidden" style="width:100%;">
                    <div style="padding:14px 16px;border-bottom:1px solid #e5e7eb;">
                        ...
                        <h1 class="text-2xl font-semibold mt-1">Altin ve Emtia</h1>
                    ...
                    <div style="max-height:440px;overflow:auto;padding:6px 16px 12px;">
                        @if(empty($economyAllGold))
                            <div class="text-sm text-gray-500 py-4">Veri yok.</div>
                        @else
                            @foreach($economyAllGold as $item)
                                <div style="display:grid;grid-template-columns:1fr 120px 120px 70px 70px;gap:10px;align-items:center;font-size:13px;padding:9px 0;border-bottom:1px solid #f3f4f6;">
                                    <div>
                                        <div style="font-weight:700;">{{ $item['symbol'] ?? '-' }}</div>
                                        <div style="color:#6b7280;">{{ $item['name'] ?? '-' }}</div>
                                    </div>
                                    <div style="text-align:right;">Alis: {{ $format($item['buying'] ?? null, 2) }}</div>
                                    <div style="text-align:right;">Satis: {{ $format($item['selling'] ?? null, 2) }}</div>
                ...
                <section style="margin-top:20px;">
                    ...
                    <div class="text-center" id="economy-gold-load-wrap">
                        @if($posts->hasMorePages())
                            <button id="economy-gold-load-more" class="px-4 py-2 rounded border bg-white hover:bg-gray-50 text-sm">Daha Fazla Yukle</button>
                        @endif
                    </div>
                </section>
            </div>
...
<script>
    (function () {
        const loadBtn = document.getElementById('economy-gold-load-more');
        const feed = document.getElementById('economy-gold-feed');
        const wrap = document.getElementById('economy-gold-load-wrap');
        if (!loadBtn || !feed || !wrap) return;

        let nextPage = {{ $posts->hasMorePages() ? (int) ($posts->currentPage() + 1) : 'null' }};
        let pending = false;
        const baseUrl = @json(route('category.economy.gold', $category->slug));
